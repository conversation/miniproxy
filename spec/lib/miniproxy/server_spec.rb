...
RSpec.describe MiniProxy do
  ...
  before do
    allow(MiniProxy).to receive(:remote).and_return remote_stub
  end
  ...
  describe ".host" do
    it "returns '127.0.0.1'" do
      expect(MiniProxy.host).to eql "127.0.0.1"
    end
  end
  ...
  describe ".stub_request" do
    it "calls stub_request on the remote object with the given arguments" do
      ...
      MiniProxy.stub_request(method: "GET", url: "namco.co.jp", response: { body: "765" })
    end
  end
end
